...
- 员工规则引擎配置项包括：押金金额、首次提现最低保留、首次提现需接单满 N 天、退店冷却期、押金不退限制、自动冻结周期、体验单结算冻结周期、普通单结算冻结周期。
...
- 普通退店保证金规则以后端 `getStaffExitPreview` 为准：入店天数不足规则里的押金不退限制，或有效接单量少于 50 单，保证金不退；即使入店天数和接单量达标，只要保证金未缴满规则阈值，也不退保证金。
- 有效接单量口径：已接单、未拒单，且派单状态为 `COMPLETED` 或 `ARCHIVED`。
- 退店时若保证金未缴满且本次不退，系统会先从可用余额中补扣保证金缺口；补扣最多扣到可用余额为 0，不允许把账户余额扣成负数。前端退店弹窗需展示保证金阈值、有效接单量、未缴满补扣金额、余额不足未补齐保证金、退店后预计可用余额。
...
- 提现仅校验已进入缴费日前 1 天（24 小时）的设备租赁费；提前生成的未来账单不限制提现。未生成但已进入该窗口的下月账单仍需预留，提现后总资产不足时拒绝申请。
...
- 订单结算收益冻结周期按结算对象逐人匹配员工标签规则，不使用统一全局配置；同一订单中不同结算员工可以拥有不同解冻周期。体验单/福袋读取 `settlementFreezeExperienceDays`，普通单读取 `settlementFreezeRegularDays`，字段缺失或异常时分别兜底 3 天和 7 天。
